          ...
          <Box className={classes.bottomBox}>
            {props.update && (
              <Button
                color={"secondary"}
                variant={"contained"}
                onClick={() => deleteGroup(state, props.callback)}
              >
                Delete
              </Button>
            )}
            <Button
              onClick={() =>
                submitQuiz(state, resetState, props.update, props.callback)
              }
              color={"primary"}
              variant={"contained"}
            >
...
async function submitQuiz(state, resetState, isUpdate, callback) {
  let quizGroupDocumentReference = firestore
    .collection("quizGroups")
    .doc(state.title);
  ...
  if (state.update)
    oldGroupDocumentReference = firestore
      .collection("quizGroups")
      .doc(state.original_title);

  try {
    await quizGroupDocumentReference.set(data);
    if (state.original_title !== state.title && state.original_title.length > 0)
      await oldGroupDocumentReference.delete();
...
async function submitFile(video) {
  if (video !== undefined) {
    let videoRef = ref.child("quizVideos/" + video.name);
    let metadata = {

...
async function setCompleted(
  ...
  if (userDoc.results[blockGroupTitle][blockTitle]["completed"] === undefined)
    userDoc.results[blockGroupTitle][blockTitle]["completed"] = [
      quizGroupTitle,
    ];
  else if (
    !userDoc.results[blockGroupTitle][blockTitle]["completed"].includes(
      quizGroupTitle
    )
  )
    userDoc.results[blockGroupTitle][blockTitle]["completed"].push(
      quizGroupTitle
    );
  else return;
...
function GroupContent(props) {
  ...
  async function setURL(uploadURL) {
    ...
    if (userDoc.result === undefined) userDoc.result = {};
    ...
    if (
      userDoc.result[blockGroupTitle][blockTitle][quizGroupTitle] === undefined
    )
      userDoc.result[blockGroupTitle][blockTitle][quizGroupTitle] = {};

    userDoc.result[blockGroupTitle][blockTitle][quizGroupTitle]["video_url"] =
      uploadURL;
  ...
  function stopRecording() {
    const tempFunc = async () => {
      await recorder.stopRecording(async function () {
        ...
        let uploadURL = `results/${localStorage.getItem(
          "hash"
        )}/${blockGroupTitle}/${blockTitle}/${quizGroupTitle}_description.webm`;
  ...
  useEffect(() => {
    ...
    async function getGroupDetails() {
      ...
      if (
        uData.results !== undefined &&
        uData.results[blockGroupTitle] != undefined &&
        uData.results[blockGroupTitle][blockTitle] != undefined &&
        uData.results[blockGroupTitle][blockTitle][quizGroupTitle] !== undefined
      )
        completedQuiz = Object.keys(
          uData.results[blockGroupTitle][blockTitle][quizGroupTitle]
        );
    ...
    <Box className={classes.groupBox} width={3 / 5} mx="auto" my={3}>
      ...
      {state.content.map((element, index) => (
        <QuizCard
          ...
          disabled={
            index < state.userData.length || index > state.userData.length
          }
...
function getText(index, list) {
  if (index < list.length) return "Completed";
  else if (index > list.length) return "Locked";
  else return "Enter";
}

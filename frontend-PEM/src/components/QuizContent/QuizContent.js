...
class QuizContent extends Component {
        ...
        <Box margin={2}>
          <Box display="flex" style={{ justifyContent: "space-between" }}>
            ...
            {element.recording && !element.url && (
              <Button
                style={
                  this.state.recording !== element.title
  ...
  async submit(timeout) {
    ...
    if (doc_data.results === undefined) doc_data.results = {};

    if (
      doc_data.results[this.state.location_data.blockGroupTitle] === undefined
    )
      doc_data.results[this.state.location_data.blockGroupTitle] = {};

    if (
      doc_data.results[this.state.location_data.blockGroupTitle][
        this.state.location_data.blockTitle
      ] === undefined
    )
      doc_data.results[this.state.location_data.blockGroupTitle][
        this.state.location_data.blockTitle
    ...
    if (
      doc_data.results[this.state.location_data.blockGroupTitle][
        this.state.location_data.blockTitle
      ][this.state.location_data.quizGroupTitle] === undefined
    )
      doc_data.results[this.state.location_data.blockGroupTitle][
        this.state.location_data.blockTitle
      ][this.state.location_data.quizGroupTitle] = {};

    doc_data.results[this.state.location_data.blockGroupTitle][
      this.state.location_data.blockTitle
    ][this.state.location_data.quizGroupTitle][
      this.state.location_data.quizTitle
    ] = {
  ...
  async uploadRecording(group) {
    ...
    const upload = async () => {
      for (var element of recordings) {
        let uploadURL = `results/${localStorage.getItem("hash")}/${
          this.state.location_data.blockGroupTitle
        }/${this.state.location_data.blockTitle}/${
          this.state.location_data.quizGroupTitle
        }/${this.state.location_data.quizTitle}/${element}.webm`;
                ...
                {this.state.description_recording && (
                  ...
                  >
                    ...
                    {(this.state.media_url == null ||
                      ...
                      <Button
                        style={
                          this.state.recording !== "description"
                            ? { backgroundColor: "#00e600", color: "white" }
                            : { backgroundColor: "#c40000", color: "white" }
                        }
                        disabled={

  ...
  async function save(blockList, title, recording, description, oTitle) {
    ...
    if (update) {
      const docUpdate = async () => {
        if (video != null)
          retObj["videoDetails"]["video_url"] = await submitFile(video);
      ...
      const newSave = async () => {
        if (video != null)
          retObj["video_details"]["video_url"] = await submitFile(video);
      ...
      <Card>
        <Accordion TransitionProps={{ unmountOnExit: true }}>
          ...
          <Box className={classes.saveButton}>
            {update && (
              <Button variant="outlined" onClick={() => downloadData(title)}>
                Download Data
              </Button>
            )}
            <Button
              onClick={() =>
                save(blockList, title, recording, description, oTitle)
              }
              variant="outlined"
              color="primary"
            >
...
async function downloadData(title) {
  ...
  resList.forEach((element) =>
    writeToTextFile(element, `${element.email}_${title}`)
  );
}
